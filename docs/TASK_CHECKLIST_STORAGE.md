...
P1b-2 仍属于 P1b，不能提前勾选：
- 完整任务编辑草稿（日期、分组、提醒、重复和清单）共用一个提交事务。
- “以后”清单修改的规则分叉、旧规则停止、新定义、当前任务清单及操作回执同事务。
- 明确区分仅改清单与改日程，保持未修改的提醒及系统注册；不直接调用会清空提醒的旧独立规则编辑路径。
- 上述操作的失败注入、冲突和幂等测试。
...
- P1a 已提交：桌面 `09682d0`、鸿蒙 `f4e9c70`。本轮 P1b-1 改动待用户确认后提交；未推送、升版、合并或发布。

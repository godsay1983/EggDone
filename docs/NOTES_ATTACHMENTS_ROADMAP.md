...
- [ ] 固定 `note-attachments.json` v1 字段、空值和大小单位。
- [ ] 固定附件元数据 Key、资产前缀和 UUID 对象路径推导规则。
- [ ] 固定 MIME 白名单、图片上限、文件上限和每条便签数量限制。
- [ ] 固定附件冲突决胜、删除墓碑和 30 天二进制保留规则。
- [ ] 固定空白草稿添加附件时的自动标题和非空校验规则。
- [ ] 建立桌面 Rust 与鸿蒙 ArkTS 共用的协议 fixture。
- [ ] 验证旧客户端继续同步 `notes.json` 时不会触碰附件对象。
- [ ] 明确 JSON 导出不包含二进制，并定义后续完整备份格式。

交付物：双端协议 fixture、对象 Key fixture、错误码和状态表。

## DA1：数据库迁移与领域模型

- [ ] 新增 `note_attachments` 表、索引和本机传输字段。
- [ ] 验证全新数据库建库。
- [ ] 验证现有数据库升级且 Todo、Note、同步设置不变。
- [ ] 新增 Attachment Rust 类型和严格参数校验。
- [ ] 实现按便签查询、创建、排序、软删除、恢复和级联软删除。
- [ ] 实现待上传、上传中、失败、已上传和仅远端状态迁移。
- [ ] 增加 Repository 和 migration 测试。

完成条件：不依赖 UI 即可稳定保存附件元数据和恢复传输状态。

## DA2：本地资产存储与图片处理

- [ ] 使用 `app_data_dir` 建立附件目录，不保存外部绝对路径。
- [ ] 安全复制原文件并以临时文件加原子重命名落盘。
- [ ] 计算 SHA-256、MIME、文件大小和图片尺寸。
- [ ] 为 JPEG、PNG、WebP 生成最大 512px JPEG 预览。
- [ ] 处理透明图片、超大尺寸、损坏文件和不支持格式。
- [ ] 实现本地文件校验、缓存命中和安全删除。
...
- [ ] 在 `s3_sync.rs` 增加字节 GET、PUT、HEAD 和受控 DELETE。
- [ ] 上传不可变对象时使用 `If-None-Match: *`。
- [ ] 保留 Content-Type、Content-Length 和 SHA-256 元数据。
- [ ] 409/412 后校验远端大小和 SHA-256，一致时按幂等成功处理。
- [ ] 下载完成后校验大小与 SHA-256。
- [ ] 对 401/403、404、409/412、超时和网络中断返回可执行错误。
- [ ] 同一附件 UUID 的传输任务去重。
...
- [ ] 新增 AttachmentSyncDocument 构建、校验、稳定排序和合并。
- [ ] 实现附件元数据 ETag、`If-Match` 和一次冲突重试。
- [ ] 远端 404 时按空文档处理并使用 `If-None-Match: *` 创建。
- [ ] 二进制全部成功后才把附件加入可上传元数据文档。
- [ ] 将 Todo、Note、附件放入同一同步互斥任务。
- [ ] 整体同步状态纳入元数据错误和待上传附件数量。
- [ ] 前台恢复、60 秒检查和手动同步覆盖附件元数据。
- [ ] 运行跨端 fixture，确认 Rust 与 ArkTS 合并结果一致。

完成条件：桌面新增图片后鸿蒙端可获取元数据和预览，失败时不误报已同步。

## DA5：图片 UI MVP

- [ ] 便签编辑器增加图片入口，不挤压颜色和现有操作。
- [ ] 实现两列图片网格、空状态、上传进度、失败和重试。
- [ ] 实现当前窗口内图片查看器和原图下载。
- [ ] 实现删除、短时撤销和排序。
- [ ] 便签卡片最多显示第一张预览和附件数量提示。
- [ ] 没有附件时保持现有卡片和编辑器布局。
- [ ] 增加拖放图片和剪贴板图片粘贴。
- [ ] 完成亮色、暗色、窄面板和键盘操作适配。

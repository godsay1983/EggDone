...
- [ ] 本地化顶部标题、同步胶囊、视图导航和新增任务区。
- [ ] 本地化全部、今天、四象限、日历、搜索和分组视图。
- [ ] 本地化 Todo 卡片、更多菜单和任务详情。
- [ ] 本地化到期、提醒、重复、重要和分组编辑控件。
- [ ] 本地化批量操作、归档、清除完成和确认对话框。
- [ ] 替换组件内硬编码 `zh-CN` 日期格式。
- [ ] 修复英文按钮、标签、空状态在窄窗口中的溢出。
- [ ] 保持当前 Todo 排序、筛选和编辑状态逻辑不变。

完成条件：用户可只使用英文完成 Todo 的新增、编辑、完成、删除、筛选和日历操作。

## DI3：便签、附件、设置与数据管理

- [ ] 本地化便签列表、编辑器、空状态和卡片操作。
- [ ] 本地化图片、文件、附件管理、同步状态和错误提示。
- [ ] 在设置页增加三段式语言选择控件。
- [ ] 本地化主题、启动、快捷键和专注时长设置。
- [ ] 本地化 S3 配置、测试连接、手动同步和冲突错误。

...
> 状态归类（2026-09-05）：DI0 至 DI5 和 DI6 自动门禁已经完成。DI6 中未勾选项是 Windows 真实 UI、托盘、通知和专注窗口的人工验收；后续开发统一转入 [NEXT_STAGE_ROADMAP.md](NEXT_STAGE_ROADMAP.md)。
...
当前自动化状态：`pnpm i18n:check` 已接入 `release:check`，覆盖 438 个中英文字典键、占位符、模板硬编码和共享快捷新增 fixture；关于页从包元数据读取当前版本，当前工程为 `1.0.7`。伪本地化字符串扩展测试已建立，但真实布局、Windows 托盘、通知、专注窗口和系统语言切换仍需按手动回归清单验收后勾选。

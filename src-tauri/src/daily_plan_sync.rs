//! Independent planning snapshots and exact revision/target receipts.
use crate::{daily_plan_protocol as protocol, daily_plan_store as store, sync_target};
...
pub fn object_key(todo: &str, occupied: &[String]) -> Result<String, String> {
    ...
    if key == todo || occupied.contains(&key) {
        return Err("PLAN_KEY_COLLISION".into());
    }
    Ok(key)
}
...
pub fn prepare(
    ...
    {
        return Err("PLAN_ETAG_REQUIRED".into());
    }
    ...
    if !sync_target::is_current(db, epoch)? {
        return Err("PLAN_CONFIG_CHANGED".into());
    }
    if remote.is_some() {
    ...
    if let Some(etag) = etag {
        tx.execute(
            "UPDATE daily_plan_sync_state SET etag=?1 WHERE id=1",
            [etag],
        )
        .map_err(error)?;
    }
...
pub fn acknowledge(
    ...
    if let Some(etag) = etag {
        ...
        return Err("PLAN_NOT_EMPTY".into());
    }
    let tx = db
        .transaction_with_behavior(TransactionBehavior::Immediate)
        .map_err(error)?;
    ...
    if matches {
        tx.execute(
            "UPDATE daily_plan_sync_state SET synced_revision=?1 WHERE id=1",

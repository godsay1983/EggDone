...
迁移本机预检查（2026-09-18，LC2a-2b-2，未提交）：两端新增只读同步状态/附件盘点和旧预览失效检查；37 组共享用例、桌面 349 项 Rust 单测、全量双端隔离 S3 回归及鸿蒙 Debug 构建通过。无新界面或正式迁移入口，不操作用户数据；下一步补实际备份证明与迁移日志绑定。见[实现与边界](docs/TASK_MIGRATION_LOCAL_PREFLIGHT.md)。前一阶段已本地提交：桌面 ca30607、鸿蒙 330b411，未推送。下文为历史阶段记录。

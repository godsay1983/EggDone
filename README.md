...
开发分支更新（2026-09-14）：已有任务“更多 → 检查清单”支持新增、编辑、勾选、排序和删除子项，保存后显示进度；取消不保存，失败保留草稿。自定义重复任务可选择“仅本次/本次及以后”，保留剩余次数与历史，未来子项默认未完成；普通重复暂仅支持本次。新建与完整字段共用草稿仍待接入。详见 [范围与验证](docs/TASK_CHECKLIST_SCOPE_UI.md) 和 [本轮 roadmap](docs/TASK_PRODUCTIVITY_ROADMAP.md)。自动化与构建不代表发布或真机验收。

...
新空间传输协议（2026-09-18，本轮未提交）：双端生产传输层接入八域格式 2 封装、空间/领域绑定、缺失对象停止和条件写入；普通同步仍阻止直接启用新空间，等待激活及终态同步完整接通。本轮无新界面，不改变现有同步设置，完整云端清空未交付。见[本轮实现与边界](docs/TASK_SYNC_SPACE_PROTOCOL.md)。上一轮已提交：桌面 f8c10f3、鸿蒙 d14afef，origin/main 已对齐；本轮未执行推送。下文保留历史记录。

...
E8b2 桌面部分未发布（2026-09-12）：已保存便签的编辑器底部新增“历史版本”，支持列表、当前/历史全文预览、二次确认恢复和重试。打开前保存草稿，恢复后重建编辑器，刷新失败时禁止返回旧输入；历史只在本机留存文字，不回退附件或关联。桌面浏览器/保存队列回归通过，鸿蒙用户入口和 E8b3 原生、真实同步验收仍待完成，不升版或推送。详见[本地历史方案、操作与验证](docs/NOTE_HISTORY_IMPLEMENTATION_PLAN.md)。

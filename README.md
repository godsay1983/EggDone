...
E7/L3c增量（2026-09-12）：L3b已本地提交（桌面eefe7cc、鸿蒙283732b）。双端普通JSON/完整备份升级数据v3，增加关联数量预览、原子恢复和本地解绑保护，继续导入旧v1/v2，见[备份契约与验证](docs/TASK_NOTE_LINK_BACKUP_CONTRACT.md)、[后续阶段](docs/TASK_NOTE_LINK_ROADMAP.md)。当前增量未提交，尚无关联按钮；下一步L3d真实跨端验收，再做L4界面。不升版、不推送，数据库schema不变。既有E6完整人工验收继续保留，下文为历史阶段记录。

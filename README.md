...
E8b1 未发布（2026-09-12）：双端数据库开始保留本机便签文字历史，每条最多 100 个版本、全库最多 1000 个；编辑、同步及导入覆盖均留存被替换文字。事务恢复内核及手机/平板原生 RDB 测试已完成，**历史入口和预览界面尚未接入**，下一步 E8b2；不恢复附件或关联，不同步历史列表，不升版或推送。详见[本地历史方案与验证](docs/NOTE_HISTORY_IMPLEMENTATION_PLAN.md)。

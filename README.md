...
E8c2-D 未发布（2026-09-12）：桌面「更多 → 统一搜索」及已保存便签编辑器可检索任务、便签和附件名称。输入后按 Enter 或点击搜索，支持分组摘要、独立分页和结果类型选择；返回保留搜索上下文，已归档任务只读，附件仅定位到管理面板，不自动打开或下载。来源/目标便签保存失败会保留输入并阻止跳转。桌面界面自动化不替代原生验收；鸿蒙搜索界面仍待 E8c2-H。详见[实现方案、验证与下一步](docs/CONTENT_SEARCH_IMPLEMENTATION_PLAN.md)。未升版或发布，不关闭此前验收门槛。

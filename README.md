...
E7/L4b未发布（2026-09-12）：双端已开放“已保存便签 → 添加 → 创建关联任务”，支持日期、独立提醒和可折叠关联任务列表。失败保留草稿、取消不创建；自动化及有限模拟器检查通过，原生确认创建/提醒/真实同步整体流程仍待验收。详见[入口、验证边界与下一步](docs/TASK_NOTE_LINK_NOTE_ENTRY.md)。未升版、未推送；下文“未开放关联按钮”等为历史阶段记录。

...
E8c1 未发布（2026-09-12）：双端统一搜索内核支持任务标题/备注、便签标题/正文和附件名称，包含分页、命中摘要及打开前目标复核。共享用例、桌面全量测试及手机/平板模拟器原生数据库测试通过；本阶段没有新的界面入口，统一搜索面板和导航将在 E8c2 接入。详见[实现方案、验证与下一步](docs/CONTENT_SEARCH_IMPLEMENTATION_PLAN.md)。未升版或发布，不关闭此前原生及用户验收门槛。

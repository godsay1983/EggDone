...
<Workspace active={linkHistory.length>0} busy={linkNavigating||linkedTaskEditing} onBack={backFromLinkedContent}>
 ...
 {:else if selectedNote}
  ...
  <Editor note={selectedNote} linkRevision={0} {scrollPositions} onChange={change} onDone={backFromLinkedContent}
...
export async function invoke(command,args){
 if(command==='list_task_note_links'){
  ...
  return ids.map(id=>({link:{uuid:'link-'+id,todo_uuid:'target-task',note_uuid:id,deleted_at:null},todo_title:'Linked task',note_title:id==='source'?'Source note':'Target note',todo_state:'active',note_state:'active',is_repeating:false}));
 }
...
const p=new URLSearchParams(location.search);setLanguageMode(p.get('lang'));document.documentElement.dataset.theme=p.get('theme');mount(Fixture,{target:document.body});
...
try {
 ...
 for(const lang of ['zh-CN','en-US'])for(const theme of ['light','dark'])for(const size of [{width:320,height:430},{width:480,height:720},{width:1000,height:760}]){
  await page.setViewportSize(size);await page.goto(server.resolvedUrls.local[0]+'__navigation?lang='+lang+'&theme='+theme);
  ...
  await page.screenshot({path:resolve(output,lang+'-'+theme+'-'+size.width+'.png')});
